...
func (ar *AlbumRepository) GetAllAlbums() ([]domain.Album, error) {
	...
	if err := ar.DB.Where("albums.generated = ?", false).Find(&albums).Error; err != nil {
		return nil, err
	}
	return albums, nil
}

func (ar *AlbumRepository) GetAllGeneratedAlbums() ([]domain.Album, error) {
	var albums []domain.Album
	if err := ar.DB.Where("albums.generated = ?", true).Find(&albums).Error; err != nil {
		return nil, err
	}
	return albums, nil
}
